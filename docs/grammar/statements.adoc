...
{tab} `*from*` `*lib*`~opt~ `_string-constant_` `_partial-import-list_` 
{tab} `*from*` `*lib*`~opt~ `_string-constant_` `*import*` `*{star}*` 
{tab} `*import*` `*lib*`~opt~ `_string-constant_` 
{tab} `*import*` `*lib*`~opt~ `_string-constant_` `*as*` `_identifier_` 

`_include-statement-component_`:
{tab} `*include*` `*lib*`~opt~ `_string-constant_` 
